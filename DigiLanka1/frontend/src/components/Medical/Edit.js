...
const Edit = () => {
  ...
  useEffect(() => {
    fetch(" http://localhost:8090/medical/get/" + params.id)
      ...
      .then((resp) => {
        ...
        DOIchange(resp.DOI);
        nicchange(resp.nic);
        passportchange(resp.passport);
        occupationchange(resp.occupation);
        agechange(resp.age);
        sexchange(resp.sex);
        phonechange(resp.phone);
        addresschange(resp.address);
        Heightchange(resp.Height);
        weightchange(resp.weight);
        blood_gchange(resp.blood_g);
        remarkschange(resp.remarks);
        raight_vchange(resp.raight_v);
        left_vchange(resp.left_v);
        squintchange(resp.squint);
        Hearingchange(resp.Hearing);
        pulsechange(resp.pulse);
        pulse_schange(resp.pulse_s);
        blood_pchange(resp.blood_p);
        heart_mchange(resp.heart_m);
        Presentchange(resp.Present);
        cnschange(resp.cns);
        pschange(resp.ps);
      })
      .catch((err) => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch("http://localhost:8090/medical/update/" + params.id, {
      method: "PUT",
